...
import page from "@/config/pages/validations/serial_repeated/edit";
...
import { getValidationSerialRepeatedRule, updateValidationSerialRepeatedRule } from "@/api/validations";
...
const SerialRepeatedEdit = () => {
  ...
  useEffect(() => {
    ...
    const preparePage = async () => {
      ...
      try {
        const response = await getValidationSerialRepeatedRule({ id: id });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await updateValidationSerialRepeatedRule(recordCurrent);
        ...
        {recordCurrent && <FormWrapper>
          <div className="row">
            ...
            <div className="col-md-6 col-12 pt-2">
              <Link href={`/validations/filtered?name=${record?.name}&value=${record?.value}&filter_type=${record?.filter_type}&origin=/validations/serial_repeated/edit&id=${record?.id}`}>{t("show_filtered_results")}</Link>
            </div>
          </div>
        </FormWrapper>}
      </form>
...
export default SerialRepeatedEdit;
