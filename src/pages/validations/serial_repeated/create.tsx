...
import page from "@/config/pages/validations/serial_repeated/create";
...
import { createValidationSerialRepeatedRule } from "@/api/validations";
...
const SerialRepeatedCreate = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await createValidationSerialRepeatedRule({ ...recordCurrent });

      setRecordCurrent(response.data.result);
      dispatch(setFlashMessage({ message: t("created_successfully"), type: "success" }));
      router.push(`/validations/serial_repeated/edit?id=${response.data.result.id}`);
    } catch (error) {
        ...
        {recordCurrent && <FormWrapper>
          <div className="row">
            ...
            {recordCurrent.name && recordCurrent.filter_type && recordCurrent.value && <div className="col-md-6 col-12 pt-2">
              <Link href={`/validations/filtered?name=${recordCurrent.name}&value=${recordCurrent.value}&filter_type=${recordCurrent.filter_type}&origin=/validations/serial_repeated/create`}>{t("show_filtered_results")}</Link>
            </div>}
          </div>
        </FormWrapper>}
      </form>
...
export default SerialRepeatedCreate;
