...
import { rowActions } from "@/config/pages/factory/settings/models/actions";
import table from "@/config/pages/factory/settings/models/table";
import page from "@/config/pages/factory/settings/models/index";
...
export const FactoryModels = () => {
  ...
  const metaColumns = [
    ...
    },
    {
      key: "name",
      cell: (row: GenericRecord) => (
        <Link className="text-primary" href={`/factory/settings/models/edit?id=${row.id}`}>
          {row.name}
        </Link>
      ),
    },
