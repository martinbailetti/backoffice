...
import { rowActions } from "@/config/pages/factory/settings/clients/actions";
import table from "@/config/pages/factory/settings/clients/table";
import page from "@/config/pages/factory/settings/clients/index";
...
export const FactoryClients = () => {
  ...
  const metaColumns = [
    ...
    },
    {
      key: "name",
      cell: (row: GenericRecord) => (
        <Link className="text-primary" href={`/factory/settings/clients/edit?id=${row.id}`}>
          {row.name}
        </Link>
      ),
    },
