...
import { rowActions } from "@/config/pages/factory/settings/types/actions";
import table from "@/config/pages/factory/settings/types/table";
import page from "@/config/pages/factory/settings/types/index";
...
export const FactoryTypes = () => {
  ...
  const metaColumns = [
    ...
    },
    {
      key: "name",
      cell: (row: GenericRecord) => (
        <Link className="text-primary" href={`/factory/settings/types/edit?id=${row.id}`}>
          {row.name}
        </Link>
      ),
    },
