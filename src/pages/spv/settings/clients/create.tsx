...
import page from "@/config/pages/spv/settings/clients/create";
...
import { createSpvClient } from "@/api/spv";


const SpvClientCreate = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await createSpvClient(recordCurrent);

      setRecordCurrent(response.data.result);
      dispatch(setFlashMessage({ message: t("created_successfully"), type: "success" }));
      router.push(`/spv/settings/clients/edit?id=${response.data.result.id}`);
    } catch (error) {
...
export default SpvClientCreate;
