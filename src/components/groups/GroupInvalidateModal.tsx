...
import { invalidateGroup } from "@/api/licenses";
...
const GroupInvalidateModal = ({
  ...
  const handleClick = async () => {
    try {
      const response = await invalidateGroup(record);
  ...
  return (
    <>
      <ModalComponent
        title={`${t("invalidate")}: ${record.GroupId}`}
        closeButton={true}
...
export default memo(GroupInvalidateModal);
