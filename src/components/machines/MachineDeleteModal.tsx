import { deleteMachine } from "@/api/licenses";
...
const MachineDeleteModal = ({
  ...
  const handleClick = async () => {
    try {
      const response = await deleteMachine(record);
  ...
  return (
    <>
      <ModalComponent
        title={`${t("delete")}: ${record.MachineId}`}
        closeButton={true}
...
export default memo(MachineDeleteModal);
