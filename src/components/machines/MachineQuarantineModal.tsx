import { quarantineMachine } from "@/api/licenses";
...
const MachineQuarantineModal = ({
  ...
  const handleClick = async () => {
    try {
      const response = await quarantineMachine(record);
  ...
  return (
    <>
      <ModalComponent
        title={`${t("set_quarantine")}: ${record.MachineId}`}
        closeButton={true}
        onClose={!updated ? () => setRecord(null) : () => endModal()}
        dismissible={false}
        onActionLabel={updated ? "" : "set_quarantine"}
...
export default memo(MachineQuarantineModal);
