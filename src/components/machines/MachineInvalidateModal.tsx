import { invalidateMachine } from "@/api/licenses";
...
const MachineInvalidateModal = ({
  ...
  const handleClick = async () => {
    try {
      const response = await invalidateMachine(record);
  ...
  return (
    <>
      <ModalComponent
        title={`${t("invalidate")}: ${record.MachineId}`}
        closeButton={true}
...
export default memo(MachineInvalidateModal);
