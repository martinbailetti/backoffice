...
import { validateMachine } from "@/api/licenses";
...
const MachineValidateModal = ({
  ...
  const handleClick = async () => {
    try {
      const response = await validateMachine(record);
  ...
  return (
    <>
      <ModalComponent
        title={`${t("validate")}: ${record.MachineId}`}
        closeButton={true}
...
export default memo(MachineValidateModal);
