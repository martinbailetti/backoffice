...
import {unsetQuarantineDevice } from "@/api/licenses";
...
const DeviceUnsetQuarantineModal = ({
    record,
    endModal,
    setRecord,
}: {
    ...
    const handleClick = async () => {
        try {
            const response = await unsetQuarantineDevice(record);
    ...
    return (
        <>
            <ModalComponent
                title={`${t("unset_quarantine")}: ${record.Id}`}
                closeButton={true}
                onClose={!done ? () => setRecord(null) : () => endModal()}
                dismissible={false}
                onActionLabel={done ? "" : "unset_quarantine"}
...
export default memo(DeviceUnsetQuarantineModal);
