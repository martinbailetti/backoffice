...
import { setQuarantineDevice } from "@/api/licenses";
...
const DeviceSetQuarantineModal = ({
    record,
    endModal,
    setRecord,
}: {
    ...
    const handleClick = async () => {
        try {
            const response = await setQuarantineDevice(record);
    ...
    return (
        <>
            <ModalComponent
                title={`${t("set_quarantine")}: ${record.Id}`}
                closeButton={true}
                onClose={!done ? () => setRecord(null) : () => endModal()}
                dismissible={false}
                onActionLabel={done ? "" : "set_quarantine"}
...
export default memo(DeviceSetQuarantineModal);
