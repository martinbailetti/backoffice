...
import {  requestDeleteDevice } from "@/api/licenses";
...
const DeviceRequestDeleteModal = ({
  record,
  endModal,
  setRecord,
}: {
  ...
  const handleClick = async () => {
    try {
      const response = await requestDeleteDevice(record);
  ...
  return (
    <>
      <ModalComponent
        title={`${t("request_delete")}: ${record.Id}`}
        closeButton={true}
        onClose={!done ? () => setRecord(null) : () => endModal()}
        dismissible={false}
        onActionLabel={done ? "" : "request_delete"}
...
export default memo(DeviceRequestDeleteModal);
